#include "..\..\include\Quest\Utility\Input.h"
DISABLE_WARNINGS
#include <xinput.h>
ENABLE_WARNINGS
...
b8 PressedOnlyOnce[KeyCount] = { 0 };

b8 PressedToggle[KeyCount] = { 0 };
b8 ReleasedToggle[KeyCount] = { 0 };

b8 KeyPressed(Key key, KeyPressMode mode)
{
	if (mode == Single)
	{
		if (keyStates[key] && (PressedOnlyOnce[key] == false))
		{
			PressedOnlyOnce[key] = true;
			return true;
		}

		if (!keyStates[key])
		{
			PressedOnlyOnce[key] = false;
			return false;
		}
	}
	...
	{
		if (keyStates[key] && (PressedToggle[key] == false))
		{
			PressedToggle[key] = true;
			ReleasedToggle[key] = !ReleasedToggle[key];
			return ReleasedToggle[key];
		}
		else if (!keyStates[key])
		{
			PressedToggle[key] = false;
			return ReleasedToggle[key];
		}
	}
	return false;
