...
type Array_##type##GetValue(Array_##type* a, u32 index);                       \
  ...
  \
void Array_##type##SetPtr(Array_##type* a, u32 index, type* value);            \
  \
...
void Array_##type##PushBack(Array_##type* a, type item);                       \
  \
void Array_##type##PushBackPtr(Array_##type* a, type* item);                   \
  \
void Array_##type##Dele(Array_##type* a);                                      \
  \
b8 Array_##type##Empty(const Array_##type* a);                                 \
  \
type * Array_##type##Begin(Array_##type* a);                                    \
  \
type * Array_##type##End(Array_##type* a);                                      \
  \
type * Array_##type##Back(Array_##type* a);                                     \
  \
...
{                                                                         \
  ...
  \
\
forceinline void Array_##type##SetPtr(Array_##type* a, u32 index, type* value) \
  \
{                                                                         \
    copyMemory(value, Array_##type##GetPtr(a, index), sizeof(type));           \
  \
}                                                                         \
  \
...
{                                                                         \
  ...
  \
\
forceinline void Array_##type##PushBackPtr(Array_##type* a, type* item)        \
  \
{                                                                         \
    if (a->length == a->capacity)                                              \
      Array_##type##Grow(a, 0);                                                \
    Array_##type##SetPtr(a, a->length++, item);                                  \
  \
}                                                                         \
  \
...
{                                                                         \
  ...
  \
forceinline type * Array_##type##Begin(Array_##type* a)                         \
  \
{                                                                         \
    return a->data;                                                            \
  \
  ...
  \
forceinline type * Array_##type##End(Array_##type* a)                           \
  \
{                                                                         \
  ...
  \
forceinline type * Array_##type##Back(Array_##type* a)                          \
  \
{                                                                         \
...
{                                                                         \
  ...
  \
forceinline u32 Array_##type##Append(Array_##type* a, const type* items,       \
                                     u32 count)                                \
  \
{                                                                         \
    if (a->capacity <= a->length + count)                                      \
      Array_##type##Grow(a, a->length + count);                                \
    copyMemory(items, a->data + a->length, count * sizeof(type));              \
...
{                                                                         \
    ...
    if (capacity < a->length)                                                  \
      Array_##type##Resize(a, capacity);                                       \
                                                                               \
    type* data = NULL;                                                         \
    data = ( type * ) spawn(capacity * sizeof(type));                                     \
	if (a->data != NULL)\
	{\
