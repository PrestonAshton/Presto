...
FindResult HashmapInternalFindByEntry(const Hashmap *hashmap,
                                      HashmapEntry *entry);
...
const HashmapEntry *HashmapInternalFindNext(const Hashmap *hashmap,
                                            HashmapEntry *entry);
...
forceinline void *HashmapGetDefault(const Hashmap *hashmap, Key key,
                                    void *defaultValue) {
  usize index = HashmapInternalFindOrFail(hashmap, key);
...
forceinline void HashmapInternalErase(Hashmap *hashmap,
                                      const FindResult *foundResult) {
  if (foundResult->dataPreviousIndex == InvalidHashmapIndex)
    ArrayGetValue(&(hashmap->hashes), foundResult->hashIndex, usize) =
        ArrayGetType(&(hashmap->data), foundResult->dataIndex, HashmapEntry)
            ->next;
  else
    ArrayGetType(&(hashmap->data), foundResult->dataPreviousIndex, HashmapEntry)
        ->next =
        ArrayGetType(&(hashmap->data), foundResult->dataIndex, HashmapEntry)
            ->next;
  ...
  ArrayGetValue(&(hashmap->data), foundResult->dataIndex, HashmapEntry) =
      ArrayGetValue(&(hashmap->data), hashmap->data.length - 1, HashmapEntry);

  FindResult last = HashmapInternalFindByKey(
      hashmap,
      ((HashmapEntry *)(ArrayGet(&hashmap->data, foundResult->dataIndex)))
          ->key);

  if (last.dataIndex == InvalidHashmapIndex)
    ArrayGetValue(&(hashmap->hashes), last.hashIndex, usize) =
        foundResult->dataIndex;
  else
    ArrayGetType(&(hashmap->data), last.dataIndex, HashmapEntry)->next =
        foundResult->dataIndex;
}
...
forceinline FindResult HashmapInternalFindByKey(const Hashmap *hashmap,
                                                Key key) {
  FindResult fr;
  ...
  while (fr.dataIndex != InvalidHashmapIndex) {
    ...
    fr.dataIndex =
        ArrayGetType(&(hashmap->data), fr.dataIndex, HashmapEntry)->next;
  }

  return fr;
}
forceinline FindResult HashmapInternalFindByEntry(const Hashmap *hashmap,
                                                  HashmapEntry *entry) {
  FindResult fr;
  fr.hashIndex = InvalidHashmapIndex;
  fr.dataIndex = InvalidHashmapIndex;
...
forceinline usize HashmapInternalMake(Hashmap *hashmap, Key key) {
  ...
  if (fr.dataPreviousIndex == InvalidHashmapIndex)
    ...
    ArrayGetType(&(hashmap->data), fr.dataPreviousIndex, HashmapEntry)->next =
        index;
...
forceinline const HashmapEntry *HashmapInternalFindFirst(const Hashmap *hashmap,
                                                         Key key) {
  usize index = HashmapInternalFindOrFail(hashmap, key);
  if (index == InvalidHashmapIndex)
    return NULL;
...
forceinline const HashmapEntry *HashmapInternalFindNext(const Hashmap *hashmap,
                                                        HashmapEntry *entry) {
  if (!entry)
    return NULL;
